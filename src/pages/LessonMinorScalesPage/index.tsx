...
const LessonMinorScalesPage = () => {
  ...
  useEffect(() => {
    api
      .post("", {
        query: `query Lesson {
            lessons(where: {title: "O que são as escalas menores?"}) {
              id
              title
              primaryContent  
              secondTitle
  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center bg-gray3">
      ...
      <div className="w-4/5 flex justify-between text-text font-medium text-base mt-10">
        ...
        <button onClick={() => navigate("/lesson/harmonic-field")}>
          Proxima Pagina
        </button>
      </div>
...
export default LessonMinorScalesPage;
